...
class DrillRivetEnv(gym.Env):
    ...
    def compute_reward_terminate(self, action, p_ee, p_hand, theta, theta_y, currentStep):
        ...
        if collided:
            terminated = True
            r_ang += 0.5 * self.bowl(theta, np.deg2rad(15)) + 0.3 * self.bowl(theta_y, np.deg2rad(12))
        ...
        if self.currentStep >= self.maxSteps:
            truncated = True
            reward += 0.5 * self.bowl(theta, np.deg2rad(15)) + 0.3 * self.bowl(theta_y, np.deg2rad(12))
            reward -= 10.0
