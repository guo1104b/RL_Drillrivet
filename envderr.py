...
class DrillRivetEnv(gym.Env):
    ...
    def __init__(self, port):
        ...
        self.total_steps = 0
        self.k_curriculum_steps = 500000
    ...
    def step(self, action):
        ...
        reward, terminated, truncated, info = self.compute_reward_terminate(action, p_ee, p_hand, theta, theta_y,
                                                                            self.currentStep)
    ...
    def parse_state(self, state):
        ...
        q = state[idx:idx + 6];
        idx += 6
        qdot = state[idx:idx + 6];
        idx += 6
        p_ee = state[idx:idx + 3];
        idx += 3
        # ori_ee = state[idx:idx + 3]; idx += 3
        target_pose = state[idx:idx + 3];
        idx += 3
        theta = state[idx:idx + 1];
        idx += 1
        theta_y = state[idx:idx + 1];
        idx += 1
        p_h = state[idx:idx + 2];
        idx += 2
        u_h = state[idx:idx + 1];
        idx += 1
        p_hand = state[idx:idx + 3];
        idx += 3
        u_hand = state[idx:idx + 1];
        idx += 1
