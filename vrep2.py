...
class VrepInterface:
    ...
    def setup_handles(self):
        ...
        ik_names = ["/ik1", "/ik2", "/ik3", "/ik4", "/ik5", "/ik6"]
        self.ik = [self.sim.getObject(name) for name in ik_names]
    ...
    def if_collision(self):
        if self.sim.getIntegerSignal('Collision') or self.sim.getIntegerSignal('Collision3'):
            print("Collision!")
            self.collision = True
            self.sim.clearIntegerSignal('Collision')
            self.sim.clearIntegerSignal('Collision3')
        return self.collision

    def reset_bill2(self, pls=3):
        self.clear_runner_queue()
        self.runner.enqueue(do_signal='Bill_do_ResetHand', done_signal='Bill_ResetHand_Done', timeout=10.0)

        pose = [0.931, -2.098, 1.365, 0.0, 0.0, 0.0, 1.0]  # [x,y,z, qx,qy,qz,qw]
    ...
    def reset_robot(self, q):
        ...
        while True:
            ...
            for j, qq in zip(self.joints, q):
                ...
                err = abs(self.sim.getJointPosition(j) - qq)
                errs.append(err)
            err = max(errs)
            timeout = 10.0
    ...
    def move_bill_init(self, ang, dist, pls, k):
        self.runner.enqueue(
            do_signal='Bill_do_StepForward',
            done_signal='Bill_StepForward_Done',
            ints={'Bill_StepForward_K': k},
            timeout=30.0,
        ...
        if ang == 0:
            ...
            self.runner.enqueue(
                do_signal='Bill_do_Rotate',
                done_signal='Bill_Rotate_Done',
                ints={'Bill_Rotate_K': k, 'Bill_Rotate_Angle': - ang},
                phase_id=1,
                target_id=pls
            )
        ...
        self.runner.enqueue(
            ...
            timeout=50.0,
            ints={'Bill_MoveHand_Dummy': self.places[pls-1], 'Bill_MoveHand_K': k},
            phase_id=2,
            target_id=pls
        )

    def move_bill(self, pls, k):
        ...
        self.runner.enqueue(
            ...
            timeout=30.0,
            ints={'Bill_StepSideways_K': k},
            strings={'Bill_StepSideways_StepSize': str(0.23*(3-pls))},  # + random
            phase_id=4,
            target_id=pls,
        )
        ...
        #     do_signal='Bill_do_Rotate',
        #     done_signal='Bill_Rotate_Done',
        #     ints={'Bill_Rotate_K': k, 'Bill_Rotate_Angle': 90},
        #     phase_id=1,
        #     target_id=pls
        # )
        # self.runner.enqueue(
        ...
        #     strings={'Bill_StepForward_StepSize': str(0.23*(pls - 3))},
        #     phase_id=0,
        #     target_id=pls
        # )
        # self.runner.enqueue(
    ...
    def move_bill_handsdown(self, pls, k):
        self.runner.enqueue(
            do_signal='Bill_do_MoveHand',
            done_signal='Bill_MoveHand_Done',
            timeout=50.0,
            ints={'Bill_MoveHand_Dummy': self.place_init, 'Bill_MoveHand_K': k},
            phase_id=3,
            target_id=pls
        )
        while self.runner.is_busy() or self.runner.queue:
    ...
    def move_bill2(self, pls0, pls, k):
        # self.runner.enqueue(
        #     do_signal='Bill_do_MoveHand',
        #     done_signal='Bill_MoveHand_Done',
        #     timeout=50.0,
        ...
        self.runner.enqueue(
            ...
            timeout=30.0,
            ints={'Bill_StepSideways_K': k},
            strings={'Bill_StepSideways_StepSize': str(0.23*(pls0-pls))},  # + random
            phase_id=4,
            target_id=pls,
        )
        self.runner.enqueue(
    ...
    def move_to_position(self, ik):
        self.sim.clearIntegerSignal('Waypoint_Done')
        self.sim.setIntegerSignal('wpHandle',ik)
        self.sim.setIntegerSignal('Do_Waypoint',1)
        while True:
            self.sim.step()
            if self.sim.getIntegerSignal('Waypoint_Done')==1:
                self.sim.clearIntegerSignal('Waypoint_Done')
                break
